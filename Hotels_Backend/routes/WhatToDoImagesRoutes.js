...
router.post(
  ...
  (req, res) => {
    ...
    req.pool.query(insertQuery, [Image, visible], (error, results) => {
      ...
      res
        .status(201)
        .json({ message: "What To Do Image created successfully" });
    });
  }
);
...
router.get("/whattodoimages/:id", (req, res) => {
  ...
  req.pool.query(query, [req.params.id], (error, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "What To Do Image entry not found" });
    }
    res.json(results[0]);
  });
});
    ...
    req.pool.query(countVisibleQuery, (error, countResults) => {
      ...
      if (visible === 1) {
        ...
        if (currentVisibleCount >= 1) {
          return res.status(400).json({
            message:
              "Cannot set visibility to 1. Maximum of 1 images can be visible.",
          });
        }
      }
      ...
      if (visible === 0) {
        ...
        if (currentVisibleCount <= 1) {
          return res.status(400).json({
            message:
              "Cannot set visibility to 0. At least 1 images must be visible.",
          });
        }
      }
...
router.put(
  ...
  (req, res) => {
    ...
    console.log(
      `Turning off visibility for ID: ${visibleOffId}, Turning on visibility for ID: ${visibleOnId}`
    );
