...
router.post("/banners", verifyToken, checkRole(["Admin"]), (req, res) => {
  const { image, head, pages_page_id } = req.body;

  // Check if the page_id exists in the pages table
  const checkPageQuery = "SELECT COUNT(*) AS count FROM pages WHERE page_id = ?";
  req.pool.query(checkPageQuery, [pages_page_id], (error, results) => {
    ...
    req.pool.query(insertQuery, [image, head, pages_page_id], (error, results) => {
      if (error) {
...
router.put("/banners/:id", verifyToken, checkRole(["Admin"]), (req, res) => {
  ...
  req.pool.query(checkPageQuery, [pages_page_id], (error, results) => {
    ...
    req.pool.query(updateQuery, [image, head, pages_page_id, req.params.id], (error, results) => {
      if (error) {
