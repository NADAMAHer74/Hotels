...
const About = () => {
  ...
  const aboutImgs = useSelector((state) => state.about.aboutUsImgs);
  const aboutContent = useSelector((state) => state.about.aboutUsContent);
  const aboutStats = useSelector((state) => state.about.aboutUsStats);
  const whatWeDoImg = useSelector((state) => state.about.whatWeDoImg);
  const whatWeDo = useSelector((state) => state.about.whatWeDo);
  const aboutServices = useSelector((state) => state.about.aboutServices);
  useEffect(() => {
          ...
          <Row className="align-items-center">
            <Col xl={6} lg={6}>
              ...
              >
                <div className="aboutThumbBox d-flex flex-column">
                  {aboutImgs.map(
                    (aboutImgData, index) =>
                      aboutImgData.visible === 1 &&
                      index <= 1 && ( // Only render if visible is 1
                        <div
                          className={`aboutThumb${
                            index + 1
                          } text-center text-sm-end`}
                          key={aboutImgData.AboutUsImages_ID}
                        >
                          <img
                            src={`http://localhost:1000/${aboutImgData.Image}`}
                            alt={`About`}
                          />
                        </div>
                      )
                ...
                <div className="aboutThumbSingle text-center text-sm-start">
                  {aboutImgs.map(
                    (aboutImgData, idx) =>
                      aboutImgData.visible === 1 &&
                      idx === 2 && ( // Only render if visible is 1 and index is 2
                        <img
                          key={aboutImgData.AboutUsImages_ID}
                          src={`http://localhost:1000/${aboutImgData.Image}`}
                          alt={`About`}
            ...
            <Col xl={6} lg={6}>
              <div className="aboutTitleBox">
                <span className="aboutTitle text-capitalize d-inline-block">
                  About Us
                </span>
                      ...
                      <Col xl={6} lg={6} md={6} sm={6} key={index}>
                        <div className="aboutService">
                          <div className="aboutServiceIcon">
                            <span>
                              <img
                                src={`http://localhost:1000/${service.Icon}`}
                                width="50"
                                height="50"
                                alt=""
                              />
                          ...
                          <div className="aboutServiceDescription">
                            <h3 className="aboutServiceTitle">
                              {service.Head}
                            </h3>
                            <p>{service.Body}</p>
                          </div>
              ...
              <Col xl={5} lg={5} key={img.WhatToDoImage_ID}>
                <div className="chooseUsThumbImg position-relative ">
                  <div className="chooseUsThumb text-center text-lg-end">
                    <img
                      src={`http://localhost:1000/${img.Image}`}
                      alt="Woman in the coast"
                    />
                  </div>
                </div>
              </Col>
